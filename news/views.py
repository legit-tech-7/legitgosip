...
from .models import ContactMessage, MusicTrack, Post, Category, VideoNews
...
def music_list(request):
    ...
    paginator = Paginator(music_tracks, 7)
...
def video_news_detail(request, slug):
    ...
    # Get related videos (4 most recent videos from the same category)
    related_videos = VideoNews.objects.filter(
        category=video.category,
        is_published=True
    ).exclude(id=video.id).order_by('-publish_date')[:4]
